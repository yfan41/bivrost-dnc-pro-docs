...
本说明书规定 DNC Pro 数控程序管理系统的安装配置方法、操作方法、参考资料与故障处置措施，
作为该系统的随机技术文件使用。

## 1.2 适用读者 {#audience}

本说明书适用于使用单位的系统管理员、工艺工程师、工艺主管、设备管理人员、
数控机床操作人员与 IT 运维人员。各岗位对应的条款索引见 [5 章导言](/guide/)。
...
DNC Pro 是部署在使用单位车间内的 CNC 加工程序管理系统。它架设于一台或多台
...
c) **不提供云端服务。** 全套系统运行于使用单位的局域网内，可在完全断网条件下运行
   （本地账号 + 本地 Postgres + 本地文件存储）。
